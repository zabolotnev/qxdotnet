...
{
    /// <summary>
    /// A table model that loads its data from a backend.  Only a subset of the available rows, those which are within or near the currently visible area, are loaded. If a quick scroll operation occurs, rows will soon be displayed using asynchronous loading in the background. All loaded data is managed through a cache which automatically removes the oldest used rows when it gets full.  This class is abstract: The actual loading of row data must be done by subclasses.
    /// </summary>
    public partial class Remote : qxDotNet.UI.Table.Model.Abstract
    {
        ...
        /// <summary>
        /// Whether to block remote requests for the row count while a request for the row count is pending. Row counts are requested at various times and from various parts of the code, resulting in numerous requests to the user-provided _loadRowCount() method, often while other requests are already pending. The default behavior now ignores requests to load a new row count if such a request is already pending. It is therefore now conceivable that the row count changes between an initial request for the row count and a later (ignored) request. Since the chance of this is low, the desirability of reducing the server requests outweighs the slight possibility of an altered count (which will, by the way, be detected soon thereafter upon the next request for the row count). If the old behavior is desired, set this property to false.
        /// </summary>
        ...
        /// <summary>
        /// Whether to clear the cache when some rows are removed. If true the rows are removed locally in the cache.
        /// </summary>
